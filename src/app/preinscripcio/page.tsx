...
import {DEFINITIVE_ACTIVITIES,FIRST_PAYMENT_MONTH,getDefinitiveActivity} from '@/lib/activity-config';
...
const ACTIVITIES:any=DEFINITIVE_ACTIVITIES;
function activityConfig(name:string){return getDefinitiveActivity(name)}
...
export default function Preinscripcio(){
 ...
 function pdfDate(value:string){if(!value)return '—';const parts=value.split('-');return parts.length===3?`${parts[2]}/${parts[1]}/${parts[0]}`:value}
 async function htmlStylePdf(){
  if(!sent)return;
  ...
  const header=(title:string,copy:string)=>{
   ...
   doc.setTextColor(ink);doc.setFont('Outfit','bold');doc.setFontSize(14);doc.text('AFA Escola Sant Salvador',M+28,13);
   ...
   doc.setTextColor(ink);doc.setFont('Outfit','bold');doc.setFontSize(15);doc.text(title,M,38);
  ...
  const section=(title:string,y:number)=>{doc.setTextColor(ink);doc.setFont('Outfit','bold');doc.setFontSize(10.5);doc.text(title,M,y);doc.setDrawColor('#999999');doc.setLineWidth(.3);doc.line(M,y+2,W-M,y+2);return y+9;};
  const field=(label:string,value:any,x:number,y:number,w:number,h=12)=>{doc.setDrawColor(line);doc.setFillColor(255,255,255);doc.roundedRect(x,y,w,h,1.5,1.5,'FD');doc.setTextColor(muted);doc.setFont('Outfit','bold');doc.setFontSize(6.8);doc.text(label.toUpperCase(),x+3,y+4);doc.setTextColor(ink);doc.setFont('Outfit','normal');doc.setFontSize(8);const lines=doc.splitTextToSize(text(value),w-6);doc.text(lines.slice(0,2),x+3,y+8);};
 ...
 async function pdf(){
  ...
  y=section('4','CONDICIONS ECONÒMIQUES',y); field('Quota base',money(effectiveQuota),M,y,58); field('Descompte',`${money(discount)} · ${discountLabel}`,77,y,71); field('Material · octubre',cfg.extra?`+${money(f.complements)}`:'Sense suplement',151,y,39); y+=16; field('Pagament',f.mitja||'A confirmar',M,y,85); field('Període abonat','Octubre 2026',108,y,82); y+=17; d.setFillColor(light); d.roundedRect(M,y,178,16,3,3,'F'); d.setTextColor(purple); d.setFont('Outfit','bold'); d.setFontSize(8); d.text('IMPORT ABONAT · OCTUBRE 2026',M+5,y+6); d.setFontSize(14); d.text(money(total),W-M-5,y+11,{align:'right'}); y+=22;
 ...
 return <main className="container"><section className="hero"><span className="eyebrow">Nou tràmit · Curs 2026–2027</span><h1>Preinscripció en pocs passos</h1><p>Completa les dades de l’alumne/a i del representant. Secretaria podrà corregir qualsevol dada després.</p></section><section className="heroPanel formIntro"><div><b>Abans de començar</b><p>Necessitaràs les dades de contacte, les persones autoritzades i l’acceptació dels textos legals.</p></div><div className="formSteps"><span className="active">1 <small>Dades</small></span><i></i><span>2 <small>Contactes</small></span><i></i><span>3 <small>Confirmació</small></span></div></section><form className="card form formCard" onSubmit={submit} style={{marginTop:20}}>
  ...
  <div className="full discountBox"><div className="discountLine"><span>Quota i descomptes</span><b>Calculats automàticament</b></div><div className="discountLine"><span>Condició aplicada</span><b>{discountLabel}</b></div><div className="discountLine"><span>{cfg.extra?'Material de Robòtica · primera quota':'Calendari de quotes'}</span><b>{cfg.extra?`+${money(f.complements)}`:'Primera quota: octubre 2026'}</b></div><div className="totalLine row space"><span>Import que s’abona ara · octubre 2026</span><span>{money(total)}</span></div><p className="muted small" style={{margin:'10px 0 0'}}>La preinscripció inclou l’abonament de la quota d’octubre. Les quotes següents es gestionen mensualment; el material només s’afegeix a la primera quota de Robòtica.</p></div>
